...
class EditProfileCollectorViewModel(private val repository: DataRepository) : ViewModel() {
            ...
            repository.getDetailCollectorById().collect{ detailUser ->
                when(detailUser){
                    is ResultState.Success -> {
                        inputName.value = detailUser.data.collectorName.toString()
    ...
    fun saveProfile(){
        viewModelScope.launch {
            ...
            repository.updateProfileCollector(
